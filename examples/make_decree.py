...
class SampleDecree(DecreeModule):
    def compose(self, alert_email: str = "alerts@example.com", admin_slack: str = ""):
        ...
        test_action.operations.send_message(groups=[ops_group], users=[service], step_duration="3h", step_from=2, step_to=3)
        test_action.operations.send_message(groups=[ops_group], message="Test message String Macro", step_duration="{$TEST_DURATION}", step_from=5, step_to=5)
