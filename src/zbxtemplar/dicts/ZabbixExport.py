...
class ZabbixExport(Schema):
    ...
    def _assign_graphs(self):
        templates = {t.name: t for t in self.templates or []}
        hosts = {h.name: h for h in self.hosts or []}
        for raw in self.graphs or []:
            graph_items = raw.get("graph_items") or []
            if not graph_items:
                continue
            name = graph_items[0].get("item", {}).get("host")
            if name in templates:
                templates[name]._graphs.append(Graph.from_dict(raw))
            elif name in hosts:
                hosts[name]._graphs.append(Graph.from_dict(raw))